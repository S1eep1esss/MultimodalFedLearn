...
# Full
core_mimiciv_path = r'C:/internship/Research/Modality Gen/Mimic-iv/physionet.org/files/mimiciv/3.1/filter/'
# Demo
# core_mimiciv_path = r'C:/internship/Research/Modality Gen/Mimic-iv-Demo/physionet.org/files/mimic-iv-demo/2.2/'

# Define MIMIC IV Image Data Location (usually external drive)
# core_mimiciv_imgcxr_path = 'Modality Gen/Mimic-CXR-jpg'

# -- Demo --
# core_mimiciv_imgcxr_path = r'C:/internship/Research/Modality Gen/Mimic-CXR-jpg/'
# -- p10 --
core_mimiciv_imgcxr_path = r'C:/internship/Research/Modality Gen/physionet.org/files/mimic-cxr-jpg/2.1.0/'

# ตรวจสอบ current working directory ก่อน
# print("PWD:", os.getcwd())

# print(os.path.exists(r'C:/internship/Research/Modality Gen/Mimic-iv/physionet.org/files/mimiciv/3.1/hosp'))

# base_iv = r'C:\internship\Research\Modality Gen\Mimic-iv-Demo\physionet.org\files\mimic-iv-demo\2.2'
# base_iv = r'C:/internship/Research/Modality Gen/Mimic-iv/physionet.org/files/mimiciv/3.1/filter'
# for sub in ['hosp', 'icu']:
#     fld = os.path.join(base_iv, sub)
#     print("Checking:", fld)
#     if not os.path.isdir(fld):
#         print("Folder not found:", fld)
#         continue
#     for fn in os.listdir(fld):
#         if fn.endswith('.csv.gz'):
#             src = os.path.join(fld, fn)
#             dst = os.path.join(fld, fn[:-3])  # ตัด .gz
#             if not os.path.exists(dst):
#                 print("Extracting:", src, "->", dst)
#                 with gzip.open(src, 'rb') as f_in, open(dst, 'wb') as f_out:
#                     shutil.copyfileobj(f_in, f_out)

# base_cxr = r'C:/internship/Research/Modality Gen/physionet.org/files/mimic-cxr-jpg/2.1.0'
# for fn in os.listdir(base_cxr):
#     if fn.endswith('.csv.gz'):
#         src = os.path.join(base_cxr, fn)
#         dst = os.path.join(base_cxr, fn[:-3])
#         if not os.path.exists(dst):
#             print("Extracting:", src, "->", dst)
#             with gzip.open(src, 'rb') as f_in, open(dst, 'wb') as f_out:
#                 shutil.copyfileobj(f_in, f_out)
...
df_transfers = dd.read_csv(core_mimiciv_path + 'hosp/transfers.csv', assume_missing=True, dtype={'careunit': 'object', 'outtime': 'object'})
...
df_emar = dd.read_csv(core_mimiciv_path + 'hosp/emar.csv', assume_missing=True, dtype={'enter_provider_id': 'object', 'event_txt': 'object'})
df_emar_detail = dd.read_csv(
    core_mimiciv_path + 'hosp/emar_detail.csv',
    assume_missing=True,
    low_memory=False,
    dtype={
...
df_labevents = dd.read_csv(core_mimiciv_path + 'hosp/labevents.csv', 
                           assume_missing=True, dtype={'storetime': 'object', 'value': 'object', 'valueuom': 'object', 'flag': 'object', 
                                                       'priority': 'object', 'comments': 'object', 'order_provider_id': 'object'})
df_microbiologyevents = dd.read_csv(core_mimiciv_path + 'hosp/microbiologyevents.csv', 
                                    assume_missing=True, dtype={'comments': 'object', 'quantity': 'object', 
                                                                'dilution_comparison': 'object', 'order_provider_id': 'object',
                                                                'ab_name': 'object', 'dilution_text': 'object',
                                                                'interpretation': 'object','org_name': 'object'})
...
df_prescriptions = dd.read_csv(core_mimiciv_path + 'hosp/prescriptions.csv', 
                               assume_missing=True, dtype={'form_rx': 'object','gsn': 'object',
                                                           'dose_val_rx': 'object','form_val_disp': 'object'})
...
## ICU
df_d_items = dd.read_csv(core_mimiciv_path + 'icu/d_items.csv', assume_missing=True, dtype={'unitname': 'object'})
df_procedureevents = dd.read_csv(core_mimiciv_path + 'icu/procedureevents.csv', 
                                 assume_missing=True, low_memory=False, dtype={'value': 'object', 'secondaryordercategoryname': 'object', 
                                                             'totalamountuom': 'object', 'starttime': 'object','endtime': 'object',
                                                             'charttime': 'object','storetime': 'object'})
...
## CXR
df_mimic_cxr_split = dd.read_csv(core_mimiciv_imgcxr_path + 'filter/mimic-cxr-2.0.0-split.csv', assume_missing=True)
df_mimic_cxr_chexpert = dd.read_csv(core_mimiciv_imgcxr_path + 'filter/mimic-cxr-2.0.0-chexpert.csv', assume_missing=True)
try:
    df_mimic_cxr_metadata = dd.read_csv(core_mimiciv_imgcxr_path + 'filter/mimic-cxr-2.0.0-metadata.csv', assume_missing=True, dtype={'dicom_id': 'object'}, blocksize=None)
except:
    df_mimic_cxr_metadata = pd.read_csv(core_mimiciv_imgcxr_path + 'filter/mimic-cxr-2.0.0-metadata.csv', dtype={'dicom_id': 'object'})
    df_mimic_cxr_metadata = dd.from_pandas(df_mimic_cxr_metadata, npartitions=7)
df_mimic_cxr_negbio = dd.read_csv(core_mimiciv_imgcxr_path + 'filter/mimic-cxr-2.0.0-negbio.csv', assume_missing=True)
...
## CXR
# print(os.listdir(core_mimiciv_imgcxr_path + 'files'))

# df_mimic_cxr_jpg = build_mimic_cxr_jpg_dataframe(core_mimiciv_imgcxr_path + 'files', do_save=True)
# print(df_mimic_cxr_jpg.head())
# print(df_mimic_cxr_jpg.columns)
# print(df_mimic_cxr_jpg.shape)
# exit()
...
if (not 'cxrtime' in df_mimic_cxr_metadata.columns) or (not 'Img_Filename' in df_mimic_cxr_metadata.columns):
    ...
    # print("df_mimic_cxr_jpg columns:", df_mimic_cxr_jpg.columns)
    # print("df_cxr columns:", df_cxr.columns)

    # df_mimic_cxr_jpg['dicom_id'] = df_mimic_cxr_jpg['dicom_id'].astype(float).astype(int).astype(str)
    # df_cxr['dicom_id'] = df_cxr['dicom_id'].astype(str)
    
    df_cxr = pd.merge(df_mimic_cxr_jpg, df_cxr, on='dicom_id')

    if 'Note' in df_cxr.columns:
        os.makedirs('corpus', exist_ok=True)
        with open('corpus/radiology_corpus.cor', 'w', encoding='utf-8') as fout:
            for note in df_cxr['Note'].dropna():
                fout.write(note.replace('\n', ' ') + '\n')
        print('corpus/radiology_corpus.cor created!')
    else:
        print('Column Note not found in df_cxr, corpus not created')
    
    # Save
    df_cxr.to_csv(core_mimiciv_imgcxr_path + 'filter/mimic-cxr-2.0.0-metadata.csv', index=False)
    #Read back the dataframe
    try:
        df_mimic_cxr_metadata = dd.read_csv(core_mimiciv_imgcxr_path + 'filter/mimic-cxr-2.0.0-metadata.csv', assume_missing=True, dtype={'dicom_id': 'object', 'Note': 'object'}, blocksize=None)
    except:
        df_mimic_cxr_metadata = pd.read_csv(core_mimiciv_imgcxr_path + 'filter/mimic-cxr-2.0.0-metadata.csv', dtype={'dicom_id': 'object', 'Note': 'object'})
        df_mimic_cxr_metadata = dd.from_pandas(df_mimic_cxr_metadata, npartitions=7)
...
df_procedureevents = df_procedureevents.compute()

for col in ['starttime', 'endtime', 'charttime', 'storetime']:
    if col in df_procedureevents.columns:
        df_procedureevents[col] = pd.to_datetime(df_procedureevents[col], errors='coerce')
...
for category_idx, category in enumerate(sorted((df.category.astype(str).unique()))):
    ...
    for item_idx, item in enumerate(sorted(category_list.label.astype(str).unique())):
        df_patientevents_categorylabels_dict = pd.concat([df_patientevents_categorylabels_dict,
                                        pd.DataFrame([{'eventtype': 'chart', 
                                                       'category': category, 'label': item}])], ignore_index=True)
...
for category_idx, category in enumerate(sorted((df.category.astype(str).unique()))):
    ...
    category_list = df[df['category']==category]
    for item_idx, item in enumerate(sorted(category_list.label.astype(str).unique())):
        df_patientevents_categorylabels_dict = pd.concat([
            df_patientevents_categorylabels_dict,
            pd.DataFrame([{'eventtype': 'lab', 'category': category, 'label': item}])
        ], ignore_index=True)
...
for category_idx, category in enumerate(sorted((df.category.astype(str).unique()))):
    ...
    for item_idx, item in enumerate(sorted(category_list.long_description.astype(str).unique())):
        df_patientevents_categorylabels_dict = pd.concat([
            df_patientevents_categorylabels_dict,
            pd.DataFrame([{'eventtype': 'hcpcs', 'category': category, 'label': item}])
        ], ignore_index=True)
...
if 'Note' in df_cxr.columns:
    ...
    # df_cxr['Note'] = df_cxr['Note'].str.replace('\n', r'\n', regex=False)
