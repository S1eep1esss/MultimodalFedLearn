...
class Mul_module(nn.Module):
    def __init__(self, device='cpu', embed_size=512, mode='joint', modality='ecn'):
        ...
        if mode == 'joint':
            ...
            if 'n' in modality:
                in_features += embed_size
            self.cls_head = nn.Linear(in_features, 2)
        elif mode == 'late':
            if 'e' in modality:
    ...
    # def embedding(self, demo, chart, lab, procedure, time, imgs, view_points, timestamps):
        embeds = []
        ...
        # print("Classifier in_features:", self.cls_head.in_features)
        return embeds
    ...
    # def forward(self, demo, chart, lab, procedure, time, imgs, view_points, timestamps, ehr_embed=None, cxr_embed=None):
        if self.mode == 'joint':
            embeds = self.embedding(demo, chart, lab, procedure, time, imgs, view_points, timestamps, notes)
            # embeds = self.embedding(demo, chart, lab, procedure, time, imgs, view_points, timestamps)
