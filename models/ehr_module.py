...
class EHR_Embedding(nn.Module):
    def __init__(self, embed_type, varible_num=1, split_num=200, embed_size=256, device='cpu'):
        ...
        if embed_type == 'demo':
            ...
            self.race = nn.Embedding(11, embed_size)
    ...
    def forward(self, x):
        ...
        if self.embed_type == 'demo':
            ...
            '''
            demo = x.T
            anchor_age, insurance, language, marital_status, race = [x for x in demo]

            # print('anchor_age:', anchor_age)
            ...
            anchor_age -= 1
            marital_status += 1
            anchor_age = torch.clamp(anchor_age, min=0)
            marital_status = torch.clamp(marital_status, min=0)

            # print('anchor_age after -=1:', anchor_age)
            # print('marital_status after +=1:', marital_status)

            anchor_age = self.anchor_age(anchor_age)
            # print('insurance index (before embedding):', insurance)
            insurance = self.insurance(insurance)
            language = self.language(language)
            marital_status = self.marital_status(marital_status)
            race = self.race(race)
            embed = torch.stack([anchor_age, insurance, language, marital_status, race])
            ...
            var_embed = self.var(var.long())
            # print('\n------DEBUG LAB EMBEDDING------')
            # print('value:', value)
            # print('max:', value.max().item())
            # print('min:', value.min().item())
            # print('embedding shape:', self.value.weight.shape)
            # print('value unique:', value.unique())
            # print('value dtype:', value.dtype)
            # print('-------------------------------\n')
            # value_embed = self.value(value)
            value_embed = self.value(value.long())
            embed = torch.cat([var_embed, value_embed], dim=1)
            embed = self.map(embed)
...
class EHR_module(nn.Module):
    def __init__(self, embed_size=256, hidden_size=256, device='cpu'):
        ...
        self.device = device
        self.linear_reduce = nn.Linear(1024, 256)
