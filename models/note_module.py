...
class Note_module(nn.Module):
    ...
    def embedding(self, notes):
        if notes is None:
            # สมมติว่า embedding size คือ 512 (แก้ตามที่ใช้จริง)
            return torch.zeros((1, 512)).to(self.device)
        # หรือถ้า notes บางตัวเป็น None (เช่น list of None)
        if isinstance(notes, list) and all(n is None for n in notes):
            return torch.zeros((1, 512)).to(self.device)
