...
if task == 'mortality':
    ...
    # criterion = nn.CrossEntropyLoss(weight=torch.tensor([1.0, 30], dtype=torch.float)).to(device)
elif task == 'longstay':
...
def train_epoch(model, device, train_loader, optimizer):
    ...
    if mode != 'early':
        ...
        # for batch_idx, ((demo, ce_ts, le_ts, pe_ts, timestamps), (img_list, img_positions, img_times), target) in enumerate(train_loader):
            ...
            with autocast(enabled=use_amp):
                ...
                # pred = model(demo, ce_ts, le_ts, pe_ts, timestamps, img_list, img_positions, img_times)
        ...
        # for batch_idx, (ehr, cxr, target) in enumerate(train_loader):
            torch.cuda.empty_cache()
            with autocast(enabled=use_amp):
                target = target.to(device)
                kwargs = {}
...
def val_epoch(model, device, val_loader):
    ...
    if mode != 'early':
        for batch_idx, ((demo, ce_ts, le_ts, pe_ts, timestamps), (img_list, img_positions, img_times), notes, target) in enumerate(val_loader):
            ...
            with autocast(enabled=use_amp):
                ...
                pred = model(demo, ce_ts, le_ts, pe_ts, timestamps, img_list, img_positions, img_times, notes)
            # DEBUG: print shape
            print(f'batch {batch_idx} target shape:', target.shape, '| pred shape:', pred.shape)
    ...
    print('val targets:', np.unique(all_targets, return_counts=True))
    print('val preds (min, max):', np.min(all_preds), np.max(all_preds))
...
def test(model, device, test_loader, threshold = None):
    ...
    return auroc, ap, auprc, report, positive_num, conf_mat

@torch.no_grad()
def best_test(model, device, test_loader, val_loader = None, ratio = None):
    # model_path = './saved_model/best_{}_{}_model_{}.pth'.format(mode, modality, task)
    ...
    if val_loader != None:
        ...
        threshold = None
    auroc, ap, auprc, report, positive_num, conf_mat = test(model, device, test_loader, threshold)
    ...
    print("test metric -- Confusion Matrix:\n{}".format(conf_mat))
    return auroc, ap, auprc, report, positive_num, conf_mat
...
print('Class distribution:', np.bincount(train_dataset.label))