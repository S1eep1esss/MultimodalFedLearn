...
class CXR_dataset(Dataset):
    def __init__(self, data_path = 'cleaned_data',
                       img_path= r'C:\internship\Research\Modality Gen\physionet.org\files\mimic-cxr-jpg\2.1.0\files',
                       split='train',
                       regenerate=True,
                       img_shape=[224, 224],
                       transform=None,
        ...
        if regenerate:
            ...
            # self.cxr.to_csv(os.path.join(data_path, 'cxr.csv'), index=False) # cxr_labeled.csv --> cxr.csv
            # self.patient_admission.to_csv(os.path.join(data_path, 'core.csv'), index=False)  # pat_admit_labeled.csv --> core.csv
            self.cxr.to_csv(os.path.join(data_path, 'cxr_labeled.csv.csv'), index=False)
            self.patient_admission.to_csv(os.path.join(data_path, 'pat_admit_labeled.csv'), index=False)
            self.imglabel = self.cxr[label_name]
        else:
            # self.cxr = pd.read_csv(os.path.join(data_path, 'cxr.csv')).drop_duplicates() # cxr_labeled.csv --> cxr.csv
            # self.patient_admission = pd.read_csv(os.path.join(data_path, 'core.csv')) # pat_admit_labeled.csv --> core.csv
            self.cxr = pd.read_csv(os.path.join(data_path, 'cxr_labeled.csv.csv')).drop_duplicates()
        ...
        if split == 'all':
            ...
            X_train, X_test, Y_train, Y_test = train_test_split(np.arange(len(self.label)).reshape(-1,1), self.label.to_numpy(), 
    ...
    #         # print(type(row))
    #         # print(row)
    #         # img_folder = row['Img_Folder_x']
    #         img_folder = row.Img_Folder_x.strip('\\/').replace('\\', '/')
    #         # img_file = row.Img_Filename
    #         img_file = row.Img_Filename_x
    #         # img_path = os.path.join(self.img_path, img_folder[1:], img_file)
    #         img_path = os.path.join(self.img_path, img_folder[1:], img_file)
    ...
    def query(self, subject_id, hadm_id):
        ...
        img_time = []
        for row in img_meta.itertuples():
            # img_folder = row.Img_Folder_x.strip('\\/')
            img_folder = row.Img_Folder.strip('\\/')
            img_folder = img_folder.replace('\\', os.sep).replace('/', os.sep)
            # img_file = row.Img_Filename_x
            img_file = row.Img_Filename
            img_path = os.path.join(self.img_path, img_folder, img_file)
            img_path = os.path.normpath(img_path)
            # print("Trying img_path:", img_path)
            img = cv2.imread(img_path, cv2.IMREAD_GRAYSCALE)
            if img is None:
                raise FileNotFoundError(f"Image not found or cannot be read: {img_path}")
            img = cv2.resize(img, (self.img_shape[0], self.img_shape[1]))
