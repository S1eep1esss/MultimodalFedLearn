...
#         if na_num == len(x):
#             x_label = table.loc[x.index].iloc[0]['label']
#             y = table[table['label'] == x_label][subset].mean().iloc[0]
#             if pd.isna(y):
#                 y = 0
#             return x.fillna(round(y))
#         mean_val = x.mean()
#         if pd.isna(mean_val):
#             mean_val = 0
#         return x.fillna(round(mean_val))
#     out = table.groupby(by=by, as_index=False)[subset].transform(fill_func)
#     print("ยังมี NaN ใน out:", out.isna().sum())
#     print(out[subset[0]].isna().sum())
#     # critical: fill NaN again, for safety!
#     out = out.fillna(0)
#     # Do not cast to int here if still in dask
#     return out

class EHR_dataset(Dataset):
    def __init__(self, split='train', data_path = 'cleaned_data', regenerate=True, reprocess=True, task='mortality', longstay_mintime=0):
        super(EHR_dataset, self).__init__()
        self.data_path = data_path

        key_ids = dd.read_csv(os.path.join(data_path, 'haim_mimiciv_key_ids.csv')).astype('int64')
        demographic = dd.read_csv(os.path.join(data_path, 'core.csv'), dtype={'deathtime': 'object'})
        ...
        if regenerate & reprocess:
            ...
            labevents = dd.read_csv(os.path.join(data_path, 'labevents.csv'), assume_missing=True, dtype={'storetime': 'object', 'value': 'object', 'valueuom': 'object', 'flag': 'object', 
            ...
            chartevents.compute().to_csv(os.path.join(data_path, 'chartevents.csv'), index=False)
            labevents.compute().to_csv(os.path.join(data_path, 'labevents.csv'), index=False)
            procedureevents.compute().to_csv(os.path.join(data_path, 'procedureevents.csv'), index=False)

        else:
            chartevents = dd.read_csv(os.path.join(data_path, 'chartevents.csv'), assume_missing=True, low_memory=False, dtype={'value': 'object', 'valueuom': 'object'})
            labevents = dd.read_csv(os.path.join(data_path, 'labevents.csv'), assume_missing=True, dtype={'storetime': 'object', 'value': 'object', 'valueuom': 'object', 
                                                                                                          'flag': 'object', 'priority': 'object', 'comments': 'object',
        ...
        if reprocess:
            ...
            labevents = labevents.dropna(subset=['subject_id', 'hadm_id'])
            labevents[['subject_id', 'hadm_id']] = labevents[['subject_id', 'hadm_id']].astype('int64')
            ...
            demographic = demographic[['subject_id', 'hadm_id', 'anchor_age', 'insurance', 'language', 'marital_status', 'race']].drop_duplicates().reset_index(drop=True)
            # ethnicity --> race
            ...
            generate_categorial_num(self.demographic, ['insurance', 'language', 'marital_status', 'race']) # ethnicity --> race
            self.demographic['insurance_num'] = self.demographic['insurance_num'].clip(0, 2)
            self.demographic['language_num'] = self.demographic['language_num'].clip(0, 1)
            self.demographic['marital_status_num'] = self.demographic['marital_status_num'].clip(0, 4)
            self.demographic['race_num'] = self.demographic['race_num'].clip(0, 10)
            # self.demographic['anchor_age_num'] = self.demographic['anchor_age_num'].clip(0, 8)
        ...
        if task == 'mortality':
            self.all_adm_disch['label'] = self.all_adm_disch['discharge_location'].apply(
                # lambda x: (1 if x['discharge_location'] == 'DIED' else 0) if not x.empty else None, axis=1
                lambda val: None if pd.isna(val) else 1 if val == 'DIED' else 0
                )
        elif task == 'longstay':
        ...
        valid_idx = ~pd.isna(self.label)
        self.all_adm_disch = self.all_adm_disch[valid_idx].reset_index(drop=True)
        self.label = self.label[valid_idx].reset_index(drop=True)

        # print('[EHR] label count:', pd.Series(self.label.value_counts(dropna=False)))
        # print('split = ', split)

        if split == 'all':
            self.data_table, self.label = self.all_adm_disch, self.label.to_numpy()
        else:
            print("label shape:", self.label.shape, "unique:", np.unique(self.label, return_counts=True))
            X_train, X_test, Y_train, Y_test = train_test_split(np.arange(len(self.label)).reshape(-1,1), 
                                                                self.label.to_numpy(), test_size=0.20, 
                                                                random_state=0
    ...
    def __getitem__(self, index):
        ...
        demo = demo[['anchor_age_num', 'insurance_num', 'language_num', 'marital_status_num', 'race_num']]
        ...
        demo[0] = int(np.clip(demo[0] if not np.isnan(demo[0]) else 0, 0, 8))    # anchor_age_num
        demo[1] = int(np.clip(demo[1] if not np.isnan(demo[1]) else 0, 0, 2))    # insurance_num
        demo[2] = int(np.clip(demo[2] if not np.isnan(demo[2]) else 0, 0, 1))    # language_num
        demo[3] = int(np.clip(demo[3] if not np.isnan(demo[3]) else 0, 0, 4))    # marital_status_num
        demo[4] = int(np.clip(demo[4] if not np.isnan(demo[4]) else 0, 0, 10))   # race_num

        print('DEMO:', demo)
        print('insurance_num index:', demo[1])
    ...
    def get_collate(self):
        def collate_fn(samples):
            ...
            return torch.from_numpy(np.stack(demo)).long(), ce_ts, le_ts, pe_ts, timestamps, torch.LongTensor(target)
        return collate_fn
...
if __name__ == '__main__':
    ...
    x = EHR_dataset(reprocess=True, task='mortality')
